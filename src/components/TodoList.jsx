  ...
  const fetchLists = (userId) => {
    ...
    onSnapshot(q, (querySnapshot) => {
      setLists(querySnapshot.docs.map(doc => ({ id: doc.id, 
        ...doc.data(),
        creationTime: doc.data().creationTime, // Ensure creationTime is included
        lastUpdated: doc.data().lastUpdated // Ensure lastUpdated is included
       })));
    });
  };
  ...
  const addList = async () => {
    if (newListName.trim()) {
      await addDoc(collection(db, "lists"), { 
        ...
        email: user.email, 
        taskCount: 0,
        creationTime: serverTimestamp(), // Set creation time
        lastUpdated: serverTimestamp() // Also set last updated initially
      });
      setNewListName("");
    }
  };
  ...
  const addTask = async (listId) => {
    ...
    if (newTask && newTask.title.trim()) {
      const taskRef = await addDoc(collection(db, "tasks"), { 
        ...
        email: user.email, 
        listId,
        creationTime: serverTimestamp(), // Set creation time for task
        lastUpdated: serverTimestamp() // Set last updated for task
      });
